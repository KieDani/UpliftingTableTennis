...
class WASBNet(nn.Module):
    def __init__(self, in_frames, resolution, pretraining=False, classify_invisible=False):
        ...
        if pretraining:
            file_path = os.path.join(paths.weights_path, 'pretraining_blurball', 'wasb', 'model.pt')
            self.load_state_dict(torch.load(file_path, map_location='cpu')['model_state_dict'])
            print(f"Loaded pretrained WASB (hrnet w18 small) model trained on BlurBall dataset")
