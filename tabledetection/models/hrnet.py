...
class MyHRNet(nn.Module):
    def __init__(self, resolution, pretraining=False):
        super(MyHRNet, self).__init__()
        W, H = resolution
        self.number_output_channels = 13
        config = {
            "name": "hrnet",
            f"frames_in": 1,
            "frames_out": self.number_output_channels,
            "inp_height": H,
            "inp_width": W,
            "out_height": H,
            "out_width": W,
            ...
            "model_path": os.path.join(paths.weights_path, 'initialization', 'wasb', 'model.pth'),
        }

        self.model = HRNet(cfg=config)
        self.pretraining = pretraining
        assert not pretraining, "Pretraining not yet supported for HRNet in tabledetection"

        # Only load ImageNet pretraining
        model_path = os.path.join(paths.weights_path, 'initialization', 'wasb', 'model.pth')
...
if __name__ == "__main__":
    resolution = (512, 384)
    model = MyHRNet(resolution=resolution, pretraining=False)
    x = torch.randn(1, 3, resolution[0], resolution[1])
    y = model(x)
    print(y.shape)  # Should be (1, 1, 512, 384)
